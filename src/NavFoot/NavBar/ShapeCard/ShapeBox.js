...
const ShapeBox = () => {
    ...
    const getProductById = () => {    
        axios.get(`${Env.server}/api/shape/getOne/${id}`)
            .then((res) => {
                if (res.data.message === "Success") {
                    setGetDataPro(res.data.shapes[0]);
                }
            })
            .catch((err) => {
                console.log("abc====>>>", err);
    ...
    return (
        <>
            <div className="application">
                <Helmet>
                    <meta charSet="utf-8" />
                    <title>{`PentagonPackaging - ${getDataPro.shape}`}</title>
                    <link rel="canonical" href={`${Env.link}/ShapeStyle/${id}/${title}`} />
                    <meta
                        name="description"
