...
const One = () => {
    ...
    const [getRespIndus, setGetRespIndus] = useState([]);
    ...
    const getAllDataForUpdate = (industry , id) => {
    ...
    const updateAllData = (imgFileName) => {
        ...
        axios.patch(`${Env.server}/api/industry/updateData`, data)
            .then((resp) => {
                let res = resp.data.industries;
    ...
    const publishBlogNow = (imgFileName) => {
        ...
        axios.post(`${Env.server}/api/industry/addNew`, data)
            .then((res) => {
    ...
    const ConfirmedDeleteBlog = () => {
        axios.delete(`${Env.server}/api/industry/deleteOne/${deleteBlogId}`)
            .then((res) => {
        ...
        <div className="bg-white" style={{backgroundColor:"white"}} >
            <div className=" d-flex" style={{ marginTop: "100px" }} >
                ...
                <div className="container d-flex justify-content-start mt-3">
                    <button className="ButtonDesgin" onClick={() => setIndustriesModalOpen(true)} style={{ backgroundColor: "#209abc", color: "white" }}>Industries</button>
                </div>
                <div className="container d-flex justify-content-end mt-3">
                    <button className="ButtonDesgin" onClick={() => setIndustriesModalOpen(true)} style={{ backgroundColor: "#209abc", color: "white" }}>Add Industry Data</button>
                </div>
            </div>
            <div className="container-fluid">
                <div className="row">
                    {getRespIndus?.map((data, index) => (
                        <>
                                ...
                                <div class="card">
                                    <div class="row g-0" style={{ height: "80px" }}>
                                        ...
                                        <div class="col-md-8 d-flex">
                                            <h5 class="card-title d-flex">{data.title}</h5>
                                            <i className="fa fa-trash mt-2" style={{ fontSize: "23px", cursor: "pointer", color: "red", marginLeft: "40px" }} onClick={() => handleDeleteBlog(data._id)}></i>

                                            &nbsp; &nbsp; <div onClick={() => {
                                                setIndustriesModalOpen(true);
                                                setEditprodId(data?._id);
                                                setClickEditModal(true);
                                                getAllDataForUpdate(data.industry  , data._id);
                    ...
                    >
                        ...
                        <center>
                            ...
                            <Typography id="modal-title" variant="h4" component="h2">
                                {clickEditModal ? (
                                    <>
                                        <b>Update Industry Data</b>
                                    </>
                                ) : (
                                    <>
                                        <b>Add new Industry Data</b>
                                    </>
                                )}
                            </Typography>
                            <Container>
            ...
            <div>
                <Dialog open={deleteModalOpen} onClose={handleDeleteModalClose} >
                    ...
                    <DialogContent>
                        <Typography><h5>Are you sure you want to delete this item?</h5></Typography>
                    </DialogContent>
                    <DialogActions>
