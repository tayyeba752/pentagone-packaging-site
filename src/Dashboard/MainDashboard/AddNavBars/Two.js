...
const Two = () => {
    ...
    const [IdEditProd, setIdEditProd] = useState('');
    ...
    const [getRespCata, setGetRespCata] = useState([]);
    const [getRespIndustries, setgetRespIndustries] = useState([]);
    ...
    const getAllIndustries = ()=>{
        ...
        .then((res)=>{
            console.log("ab====", res.data.industries);
            setgetRespIndustries(res.data.industries);
        })
        .catch((err) => {
            console.log("abc===", err);
        })
    ...
    const getAllDataForUpdate = (catagory , id) => {
        axios.get(`${Env.server}/api/catagory/getOne/${id}`)
            .then((resp) => {
                let res = resp.data.catagori[0];
                setIdEditProd(id);
                setTitle(res.title);
                setCategory(res.category);
                setDescription(res.description);
                setDescription2(res.description2);
    ...
    const updateAllData = (imgFileName) => {
        ...
        axios.patch(`${Env.server}/api/catagory/updateData`, data)
            .then((resp) => {
                let res = resp.data.catagories;
    ...
    const handleeditBlog = (productId) => {
        seteditBlogId(productId);
    ...
    const ConfirmedDeleteBlog = () => {
        axios.delete(`${Env.server}/api/catagory/deleteOne/${deleteBlogId}`)
            .then((res) => {
        ...
        <div className="bg-white" style={{ backgroundColor: "white" }}>
            <div className=" d-flex">
                ...
                <div className="container d-flex justify-content-start mt-3">
                    <button className="ButtonDesgin" onClick={() => setIndustriesModalOpen(true)} style={{ backgroundColor: "#209abc", color: "white" }}>Catagories</button>
                </div>
                <div className="container d-flex justify-content-end mt-3">
                    <button className="ButtonDesgin" onClick={() => setIndustriesModalOpen(true)} style={{ backgroundColor: "#209abc", color: "white" }}>Add Category Data</button>
                </div>
            </div>
            <div className="container-fluid">
                <div className="row">
                    {getRespCata?.map((data, index) => (
                        <>
                                    ...
                                    <div class="row g-0" style={{ height: "80px" }}>
                                        ...
                                        <div class="col-md-8 d-flex">
                                            ...
                                            &nbsp; &nbsp; <div onClick={() => {
                                                ...
                                                getAllDataForUpdate(data.category, data._id );
                                            }
                                            } style={{ fontSize: "23px", cursor: "pointer", color: "#11a5fa", marginLeft: "30px" }} >
                                                <FaEdit />
                                            </div>
                                        </div>
                                    </div>
                    ...
                    >
                        ...
                        <center>
                            ...
                            <Typography id="modal-title" variant="h4" component="h2">
                                {clickEditModal ? (
                                    <>
                                        <b>Update Category Data</b> 
                                    </>
                                ) : (
                                    <>
                                        <b>Add new Category Data</b>
                                    </>
                                )}
                            </Typography>
                            <Container>
                                ...
                                <FormControl style={{ minWidth: '100%', marginTop: "10px" }}>
                                    ...
                                    >
                                        {getRespIndustries && getRespIndustries.map((data, index) => (
                                            <MenuItem key={index} value={data._id}>{data.title}</MenuItem>
                                        ))}
                                    </Select>
                                </FormControl>
                                ...
                                <JoditEditor
                                    ...
                                    value={description2}
                                    // config={config}
                                    onChange={newContent => setDescription2(newContent)}
                                />
